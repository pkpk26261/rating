...
只需上傳 `README.md`、`manifest.json`、`icon.svg`、`index.html`、`icon-512.png`、`icon-192.png`、`icon.png` 這七個檔案。帳號登入、同步程式、學生管理入口及介面樣式已整合進 `index.html`，不必另行上傳 `account/`、`vendor/` 或 `scripts/`。更新後重新整理網頁即可；既有 Google 網址、同步密碼與教學資料會保留。

Google 同步視窗將上傳／下載分成兩張操作卡，標題與說明分行；兩個自動同步選項各自一列，整列可點擊，窄螢幕改為單欄。下載仍需確認取代本機資料。

維護程式時，修改 `account/` 來源後執行 `node scripts/sync-account-assets.cjs`，即可重新產生完整的 `index.html`。Excel 匯入／匯出沿用網路載入的 SheetJS；Google 與帳號同步仍需要網路及已設定的雲端服務。這七個檔案不包含 Apps Script 或後端服務的部署。
...
| **座位** | 直排／分組／U 型範本、自由移動與旋轉桌子、配置講台與出入口；支援自動排位、隨機入座與跨班複製教室 |
...
學生搜尋欄只用來輸入姓名或座號，不接受瀏覽器自動填入的帳號資料；一般欄位與動態學生編輯欄位也關閉自動填入。登入帳號／密碼保留瀏覽器填入功能。搜尋按 Enter 可清除，中文輸入法選字時按 Enter 不會清除搜尋。
...
- **下載完整備份**：產生一個 JSON 檔，包含全部班級、學生與幹部、加扣分、作業分數及四種狀態、座位格數／配置／停用格、學生備註、違規與特殊記事、分組、抽號歷史、全部課程進度與勾選，以及課堂任務板、倒數計時和成績顯示設定。沒有班級但仍有課程時也可備份。支援的瀏覽器會開啟「另存新檔」，可選位置與檔名；其他瀏覽器使用一般下載並顯示下載清單提示。取消另存不會繼續下載，寫入失敗會明確提示。Google 模式同樣可備份，背景上傳中或同步失敗也不影響；備份包含目前本機的最新資料，不必先上傳，也不會重新下載雲端資料或包含同步網址與密碼。
- **還原完整備份**：選擇上述 JSON 檔或平板文字備份（.txt），先檢查格式、版本與內容，再顯示備份時間、班級、人數和課程摘要。勾選取代確認後，按「確認還原」才會取代目前全部資料；可先下載目前資料的備份，或直接關閉取消。一般文字檔不會通過完整備份格式檢查。

**iPad Web App 儲存方式**：點「下載完整備份」→「開啟儲存選單」→「儲存到檔案」，選位置後儲存。若裝置不支援 JSON 檔案分享，會提供內容相同的文字備份（.txt），可直接用本系統還原。取消與失敗會保留備份視窗，方便重試；也可點「下載 JSON 備份」。系統選單完成不代表已確認寫入，請到選擇的位置確認檔案。

備份使用目前畫面所操作的最新資料，包含尚未完成本機存檔的修改。請確認備份檔已下載完成並妥善保管。Excel／CSV 成績匯出檔供查閱使用，並非此功能的還原檔案；舊成績檔不會自動轉為完整備份。
...
一般／Google 模式的大型教學資料會自動無損壓縮；遇到容量不足時，會原地壓縮舊的教學資料並重試儲存，保留班級、記事及下載雲端前的備份，不清除 Google 連線設定或其他帳號資料。首次啟用壓縮時，其他舊分頁會重新載入以更新讀取格式。壓縮後若仍無法寫入，會繼續顯示「本機儲存失敗」，可重試或下載完整備份；壓縮不會增加瀏覽器本身的容量上限。
...
從「課堂工具 → 考試時間表」開啟，即使沒有學生名單也能使用。所有班級共用同一份考試設定。介面依原版的深藍漸層、紅色標題、黃色／紅色倒數及青色提示設計，支援深淺色。考試頁與課堂入口使用同一個筆記板＋時鐘 SVG，其餘操作使用一致的線條 SVG；早自習使用書本、午休使用月亮圖示。課堂工具選單依序為「任務版 → 倒數計時 → 考試時間表」。

- 字級、數字、圖示與按鈕依可用寬度及高度調整；起訖時間加粗，節次與科目提高對比。時鐘、倒數及人數依容器空間自動放大，視窗變矮時同步調整，操作區至少 44px。
...
- 人數、備註與考試規則可直接編輯並自動存本機；規則每行一項，支援空白內容、換行與長文捲動，不另開編輯視窗。缺席人數自動計算，最低為 0。縮小／比例／放大緊接資訊標題右側，最右側為自動輪播開關。底部兩條短線貼齊資訊卡下緣，直接點選切換人數／規則，沒有彈窗或左右按鈕。自動輪播每 8 秒切換；輸入、停留在資訊區或開啟時間編輯視窗時暫停。備註與規則可各別調整為 50%～200%。
- 節次區右上方按鈕可收合節次；收合後整欄完全隱藏，倒數與資訊區填滿空間。電腦滑鼠靠近考試內容區左側時，游標附近會浮出 SVG 箭頭，點一下即可展開；離開後 250ms 自動隱藏，不占版面空間。也保留滑鼠向右拖曳及平板從左邊界向右滑動；平板輕點空白邊界、垂直捲動和短距離誤滑不會展開。平板接滑鼠也能使用浮動箭頭。鍵盤可用 Tab 移到展開節次按鈕，再按 Enter 或空白鍵。「全螢幕」放大考試畫面；控制列與投影按鈕等高並排，考試頁的主題切換按鈕僅在投影時顯示；退出投影固定留在不捲動的頂部操作列；若瀏覽器不允許全螢幕，仍可使用頁內投影。考試頁的左右方向鍵切換資訊，F11 切換投影，Escape 退出投影；編輯文字時不攔截方向鍵。
...
## 2026-09-12 自由教室配置（本機）

「課堂工具 → 座位」現在分為 **學生換位** 與 **桌椅位置**。先選擇桌椅位置，套用一般直排、分組（4／6 位）或 U 型範本，再調整學生桌、老師桌、講台與出入口。每間教室至少保留一張老師桌；可另外新增、移動與旋轉，最後一張不能刪除。範本數量預設取目前班級人數，可在預覽中修改；第一版最多 81 張桌子。

- 桌子本體可輸入 0–359° 任意角度，調整選取內也保留每次 45° 旋轉，桌面不顯示方向箭頭；座號、姓名與加扣分按鈕反向校正，保持正向。旋轉後的實際外框用於畫布邊界與框選判斷。
- 編輯工具分為「教室設定」、「＋學生桌」、「老師桌／門」、「選取多張」；選取物件後才顯示角度、「更多調整」與「刪除選取」。點選桌子或設施後可拖動。桌機可拖曳空白處框選；平板可先按「選取多張」，逐一點選，再拖動或按「點選目的位置」。選取桌子後，直接在「角度」輸入數值。方向鍵可微調選取物件，Escape 取消；「拖動畫面」可直接切換平移；縮小、比例、放大直接顯示在工具列，「完整教室」還原縮放及捲動位置並結束平移。100% 代表整間教室完整顯示。
- 安排學生時可直接選取後點桌位交換，或按「放回未安排」；「待入座 N 人」按鈕開啟格狀名單，切班預設收納。滑鼠、觸控橫直拖曳與鍵盤 Enter／Space 都可入座。「自動安排」內的「排位起點」與「依座號安排」並排在同一組，先選四個角落之一，再按按鈕安排；重複執行沿用所選起點；「隨機填入空位」只安排待入座學生；兩種方式各有用途說明。「清空學生」直接放回名單，可復原。
- 講台可自由移動；出入口可手動新增，最多兩扇門，移動時吸附到最近的牆邊。選取設施後，在「調整選取」修改名稱。新增學生桌與出入口會避開既有桌位及設施。
- 「刪除」桌子或「停用」桌位會把該位學生放回未安排名單；「清空學生」保留教室配置。「復原／重做」保留目前操作期間最近 20 步，重新載入或外部資料取代後重設。
- 複製其他班只複製教室配置。預覽會列出保留原位與返回未安排名單的人數；目的班可對應的學生仍留在原座位，來源班學生不會被帶入，之後兩班各自獨立。
- 加扣分的「座位圖」呈現同一份教室配置；「座號排列」保持既有用途。座位圖預設依可用寬高完整顯示全班，不需上下左右捲動。空間不足時，卡片優先顯示姓名、座號與分數，點選學生後使用固定操作列的「＋1 分／－1 分」（至少 44px）；手動放大到足夠尺寸時才顯示卡片內按鈕。放大後仍可平移／捲動，「完整顯示」回到全班視野。

「教室設定 → 教室寬度／高度」可分別調整教室寬高，預覽後套用；桌位依比例調整，學生留在原桌。手動設定尺寸後，視窗縮放只影響顯示，不會覆寫教室尺寸。

資料沿用每班 `seating` 的 `rows`、`cols`、`map`、`disabled`，新增選用的 `layout`（version 2，相容 version 1 並自動補入老師桌）：畫布寬高、以原座位鍵索引的桌子座標／角度，以及老師桌／講台／門的設施資料。移動桌子不改變學生識別或原座位鍵。原試算表座位列／欄仍表示穩定座位鍵；自由位置與方向保存在完整補充快照中。

完整 JSON 備份、Google 補充快照，以及人工修改班級表後的下載重建，均保存新配置。沒有 `layout` 的舊資料以原列欄、停用格與下方講台呈現。新配置格式、旋轉後邊界或學生對應損壞時，拒絕整批匯入，保留本機資料。

驗證命令：

```sh
node --test tests/*.test.cjs
node tests/serve-workspace-preview.cjs 8789
NODE_PATH=/Users/user/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules node tests/classroom-browser.cjs
NODE_PATH=/Users/user/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules node tests/classroom-interactions-browser.cjs
NODE_PATH=/Users/user/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules node tests/classroom-rebuild-browser.cjs
```

瀏覽器驗證使用虛構學生與 localhost，Google 請求由測試頁模擬；不會寫入真實 Google 試算表。實體平板與正式發布需另行驗證。

講台尺寸：在「佈置教室」點選講台，按「調整講台大小」，拖動寬度／高度滑桿預覽後套用。尺寸隨班級保存，可復原／重做。講台與所有老師桌皆不可刪除（包含 Delete 鍵及混合多選），仍可拖曳移動。

多選桌子：在「佈置教室」按「選取多張」，逐張點選；「更多調整 → 複製整組桌子」保留相對位置、間距及角度，副本整組錯開並保持選取，可直接拖動；不複製學生。也可直接按「刪除 N 張桌子」，學生回到待入座名單，支援復原。包含講台或老師桌的選取組合不可刪除。

畫布直接調整大小：進入「佈置教室」後，拖教室左、右、上、下方細控制線，或右下角圓點。選取講台／老師桌後也有相同控制點；拖本體仍是移動。操作區保持 44px，但不顯示大按鈕或寬高文字。拖曳即時預覽、放開保存，可復原／重做；Escape 取消。鍵盤方向鍵可微調控制點。小物件控制點留出間距，避免寬、高命中區互相遮擋。

新增物件集中在「新增桌椅／門」選單，依序為學生桌、老師桌及出入口；出入口仍最多兩扇。

在「佈置教室」點选門，可直接切換「左開／右開／禁止出入」。開向切換鉸鏈與門片位置；禁止出入顯示紅色標誌。設定隨班級保存、可復原／重做，加扣分座位圖共用，舊資料保留原有開向。
